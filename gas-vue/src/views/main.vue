		...
		<nav class="site-navbar" :class="'site-navbar--' + navbarLayoutType">
			<div class="site-navbar__header">
				<h1 class="site-navbar__brand">
					<a class="site-navbar__brand-lg">智能燃气平台管理系统</a>
					<a class="site-navbar__brand-mini">燃气</a>
				</h1>
			</div>
				...
				<el-menu
						...
						>
							...
							<el-sub-menu index="组织管理" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
								...
								<el-menu-item index="部门管理" v-if="isAuth(['ROOT', 'DEPT:SELECT'])" @click="$router.push({ name: 'Dept' })">
									<SvgIcon name="company_fill" class="icon-svg" />
									<span>部门管理</span>
								</el-menu-item>
								
								<el-menu-item index="角色管理" v-if="isAuth(['ROOT', 'MIS_ROLE:SELECT'])" @click="$router.push({ name: 'Role' })">
									<SvgIcon name="role_fill" class="icon-svg" />
									<span>角色管理</span>
								</el-menu-item>
								<el-menu-item index="用户管理" v-if="isAuth(['ROOT', 'MIS_USER:SELECT'])" @click="$router.push({ name: 'User' })">
									<SvgIcon name="user_fill" class="icon-svg" />
									<span>用户管理</span>
								</el-menu-item>
							</el-sub-menu>
							<el-sub-menu index="建户立档" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
								<template #title>
									<SvgIcon name="trust_fill" class="icon-svg" />
									<span>建户立档</span>
								</template>
								<el-menu-item index="巡检人员管理" v-if="isAuth(['ROOT', 'GAS_WORKER:SELECT'])" @click="$router.push({ name: 'Worker' })">
									<SvgIcon name="worker_fill" class="icon-svg" />
									<span>巡检人员管理</span>
								</el-menu-item>
								<el-menu-item index="燃气用户管理" v-if="isAuth(['ROOT', 'GAS_USER:SELECT'])" @click="$router.push({ name: 'GasUser' })">
									<SvgIcon name="user_fill" class="icon-svg" />
									<span>燃气用户管理</span>
								</el-menu-item>
							</el-sub-menu>
							<el-sub-menu index="入户安检" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'">
								<template #title>
									<SvgIcon name="night_fill" class="icon-svg" />
									<span>入户安检</span>
								</template>
								<el-menu-item index="巡检日程表" v-if="isAuth(['ROOT', 'SCHEDULE:SELECT'])" @click="$router.push({ name: 'WorkPlan' })">
									<SvgIcon name="calendar_fill" class="icon-svg" />
									<span>巡检日程表</span>
								</el-menu-item>
								<el-menu-item index="燃气巡检计划" v-if="isAuth(['ROOT', 'SCHEDULE:SELECT'])" @click="$router.push({ name: 'GasWorkerPlan' })">
									<SvgIcon name="calendar_fill" class="icon-svg" />
									<span>燃气巡检计划</span>
								</el-menu-item>
								<el-menu-item index="隐患管理" v-if="isAuth(['ROOT', 'HAZARD_INFO:SELECT'])" @click="$router.push({ name: 'HazardInfo' })">
									<SvgIcon name="tixing" class="icon-svg" />
									<span>隐患管理</span>
								</el-menu-item>
							</el-sub-menu>
							<el-sub-menu index="系统设置" :popper-class="'site-sidebar--' + sidebarLayoutSkin + '-popper'" v-if="isAuth(['ROOT', 'SYSTEM:SELECT'])">
								<template #title>
...
export default {
	...
	created() {
		...
		if (userStr) {
			...
			that.name = user.name || user.username;
		}
